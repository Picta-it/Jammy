...
* Added script to copy static assets from ~app/assets to ~/dist during compilation
...
* Add missing alias to `~/app/store`
...
* Added `npm run lint` script to lint all `~/app` code
...
#### Improvements
* Renames `client` directory to `app` to be more intuitive.
...
#### Deprecations
* Removes local test utilities (in `~/app/utils/test`).
...
* Client application code now lives in `~/client` instead of `~/app` in order to conform to Redux standards.
...
#### Improvements
* Restructures app directory so filenames are more identifiable.
...
* Adds alias `utils` to reference `~/app/utils`
